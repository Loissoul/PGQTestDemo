...
@interface PGQContentMenuTableView ()
...
@property (nonatomic, strong) UITableViewCell<PGQContentMenuCell> *selectedCell;
@property (nonatomic, strong) NSIndexPath *dismissalIndexpath;
@property (nonatomic) AnimatingState animatingState;

@end

@implementation PGQContentMenuTableView
...
- (void)showInView:(UIView *)superview withEdgeInsets:(UIEdgeInsets)edgeInsets animated:(BOOL)animated {
    ...
    for (UITableViewCell<PGQContentMenuCell> *aCell in [self visibleCells]) {
...
- (void)dismisWithIndexPath:(NSIndexPath *)indexPath {
    ...
    for (UITableViewCell<PGQContentMenuCell> *visibleCell in visibleCells) {
...
- (void)show:(BOOL)show visibleCellsAnimated:(BOOL)animated {
    ...
    UITableViewCell<PGQContentMenuCell> *visibleCell = [visibleCells objectAtIndex:self.animatingIndex];
    if (visibleCell) {
...
- (void)dismissBottomCells {
    if (self.bottomCells.count) {
        UITableViewCell<PGQContentMenuCell> *hidingCell = [self.bottomCells lastObject];
...
- (void)dismissTopCells {
    if (self.topCells.count) {
        UITableViewCell<PGQContentMenuCell> *hidingCell = [self.topCells firstObject];
...
- (void)show:(BOOL)show
        cell:(UITableViewCell<PGQContentMenuCell> *)cell
...
#pragma mark - Utils
- (void)resetAnimatedIconForCell:(UITableViewCell<PGQContentMenuCell> *) cell{
    UIView *icon = [cell animatedIcon];
